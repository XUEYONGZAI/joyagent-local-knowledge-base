...
from genie_tool.model.protocal import TableRAGRequest, AutoAnalysisRequest, CIRequest, CalEngineRequest, ReportRequest, DeepSearchRequest, NL2SQLRequest, SopChooseRequest, KnowledgeRAGRequest
...
from genie_tool.tool.knowledge_rag import knowledge_rag_agent
load_dotenv()
...
@router.post("/knowledge_rag")
async def post_knowledge_rag(
    ...
    async def _stream():
        ...
        acc_token = 0
        acc_time = time.time()
        async for chunk in knowledge_rag_agent(
            task=body.task,
        ...
        if body.stream_mode.mode in ["time", "token"] and acc_content:
            yield ServerSentEvent(
                data=json.dumps({"requestId": body.request_id, "data": acc_content, "isFinal": False},
                                ensure_ascii=False))
        yield ServerSentEvent(data=json.dumps(
            {"requestId": body.request_id, "data": acc_content, "isFinal": True}, ensure_ascii=False))
        yield ServerSentEvent(data="[DONE]")
